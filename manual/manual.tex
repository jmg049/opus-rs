...
\begin{concept}{Why Packets Carry No Length Field}
  ...
  inside multistream packets (\cref{sec:multistream}), where several Opus packets
...
\begin{table}[h]
  ...
  \begin{tabular}{llll}
    \toprule
    \textbf{Config} & \textbf{Mode} & \textbf{Bandwidth} & \textbf{Frame sizes (ms)} \\
    \midrule
  ...
  coder runs; the bandwidth caps the highest coded frequency; the frame size is
...
\begin{table}[h]
  ...
  \begin{tabular}{lll}
    ...
    R3 & code 1 payload length is even                 & \code{Code1UnevenPayload} \\
    R4 & coded frame lengths are not truncated         & \code{InvalidFrameLength} \\
    R5 & code 3 count $M\ge 1$ and total $\le$ 120~ms  & \code{InvalidFrameCount} \\
    R6 & padding does not exceed the payload           & \code{InvalidPadding} \\
    R7 & frame lengths do not overrun the payload      & \code{InvalidFrameLength} \\
    \bottomrule
  \end{tabular}
...
the one its name advertises, is that it codes the \emph{energy} of each frequency
...
\begin{concept}{The Modified Discrete Cosine Transform}
  ...
  a single IMDCT cannot recover its input; the missing information is supplied by
  the \emph{overlapping} neighbor block. Each output block overlaps its successor
...
interior is passed through unwindowed. This \termdef{low-overlap} design trades a
...
2, 4, 8$), each covering a fraction of the frame. The encoder signals this choice
...
factors into three stages: a complex \emph{pre-rotation} that folds the $2N$ real
...
through an external planned-FFT backend, while a dependency-free build falls back
...
finely, wide at high frequencies where it does not~\cite{zwicker1999psychoacoustics}.
...
\begin{concept}{Why Code Energy Separately}
  ...
  when the shape is coded coarsely. A codec that quantized raw coefficients would
  instead let energy drift as bits run short, producing audible spectral tilt and
  ``birdie'' artifacts. The cost of the guarantee is the overhead of the envelope
...
\begin{concept}{The Pyramid Codebook}
  ...
  \end{equation}
  Geometrically these are the lattice points on the surface of an $N$-dimensional
...
The PVQ search yields a codeword $\mathbf{y}$; the coder must transmit \emph{which}
...
$\mathbf{y}$ through the table, and normalizes. Because the index is uniform over
...
(\cref{subsec:celt_bands}), and it may explicitly \emph{skip} high bands when the
...
When a band is split in two, the energy is divided between the halves by an angle
...
the total band energy exactly fixed, preserving the energy constraint through the
...
\begin{itemize}[itemsep=2pt]
  ...
  \item \textbf{Intensity stereo:} above a boundary band set by the budget, only
    the mid is coded and the side is reconstructed by copying the mid, discarding
    stereo detail where it is least audible (at high frequencies, low bitrates).
  \item \textbf{Dual stereo:} the two channels are coded fully independently, used
...
running the input through a high-pass filter and then forward and backward energy
...
\textbf{Time-frequency analysis.} For the bands, the encoder chooses time-versus-
...
\textbf{Dynamic allocation and trim.} Two analyses shape the allocation before it
...
comb \termdef{pre-filter} that whitens the signal's harmonic structure, the exact
...
kilobits per second, far below what a general transform coder achieves. SILK runs
at one of three internal rates (8, 12, or 16~kHz, for narrowband, mediumband, and
...
(\cref{subsec:silk_gains,subsec:silk_excitation}), the noise-shaping quantizer at
the encoder's heart (\cref{subsec:silk_nsq}), and the supporting machinery of
stereo, resampling, and control (\cref{subsec:silk_stereo,subsec:silk_resampler,subsec:silk_control}).
...
The frame is the unit at which SILK transmits its model; the subframe is the unit
...
method}~\cite{burg1975maximum} instead estimates reflection coefficients directly
...
\begin{concept}{Why Line Spectral Frequencies}
  ...
  affects the spectrum mainly near its own frequency, and a quantization error in
...
scheme (each residual partly predicted from the previous) and an arithmetic-coded
index. The residual quantization is weighted by \termdef{Laroia
weights}~\cite{laroia1991robust}, which approximate each coefficient's perceptual
...
For 20~ms frames, the decoder interpolates between the previous frame's NLSFs and
...
pulse train, and successive pitch periods resemble one another. SILK removes this
...
\begin{concept}{Pitch Prediction with a Five-Tap Filter}
  ...
  period. What remains after long-term prediction, the doubly-predicted residual,
...
against the bits to address it: highly periodic speech justifies the larger, more
precise codebook. The encoder chooses the lag with a three-stage pitch search
(\cref{subsec:silk_control}) and the gains with a rate-distortion-weighted vector
quantizer.
...
\begin{concept}{Recursive Pulse Splitting}
  ...
  count divides between its two children, using a probability model conditioned on
  the parent count. This is efficient because the total pulse count already
  constrains the splits: a block with two pulses has far fewer ways to distribute
...
\begin{concept}{Shaping Quantization Noise}
  ...
  prediction, so the quantizer is steered to make errors whose spectrum matches a
  perceptual target. Because the loop reconstructs exactly the signal the decoder
...
$\abs{\text{error}} + \lambda \cdot \text{bits}$, where $\lambda$ comes from the
...
\termdef{harmonic shaper} that shapes noise around the pitch harmonics. These are
...
the mid and so costs few bits. The encoder estimates per-frame prediction weights
...
long FIR for exact 2$\times$), and other ratios combine the 2$\times$ allpass with
...
final search at the internal rate locks in the per-subframe lags. The search also
...
byte budget, the gains are scaled coarser and the frame re-encoded until it fits,
...
When a SILK frame is lost, the decoder conceals it by extrapolating the last good
...
a long one decays gracefully to silence rather than repeating a buzz. This is the
...
\begin{concept}{Redundancy Frames at Transitions}
  ...
  no valid CELT history; when switching from SILK to CELT, it covers the end. The
  decoder cross-fades between the redundant frame and the primary decode over a
  2.5~ms overlap using a squared Vorbis window, so the energy sums to unity across
  the splice and no discontinuity is heard. The presence and direction of
  redundancy are coded with a few bits at the boundary of the SILK and CELT data.
\end{concept}
...
TOC packets that the decoder conceals as comfort noise. The decision is driven by
...
falls to SILK for low-bitrate speech bandwidths, hybrid for the higher bandwidths
