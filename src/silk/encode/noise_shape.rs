...
/// cosine (`win_type==2`) slope of `length` samples (multiple of 4).
...
fn autocorrelation(results: &mut [f32], input: &[f32], count: usize) {
    let n = input.len();
...
fn schur(refl_coef: &mut [f32], auto_corr: &[f32], order: usize) -> f32 {
    let mut c = [[0.0f64; 2]; MAX_SHAPE_LPC_ORDER + 1];
    for k in 0..=order {
...
fn bwexpander(ar: &mut [f32], order: usize, chirp: f32) {
    let mut cfac = chirp;
