...
/// Backwards one-tap predictive dequantiser of the stage-two residuals
/// (Q10).
...
/// Enforces the codebook's minimum distances with minimal movement; falls
/// back to a sort-and-clamp after 20 loops.
...
/// NLSF vector (Q15) from the codebook path indices.
...
/// `QA` of the polynomial construction.
const QA16: i32 = 16;

/// Builds an even/odd polynomial from the interleaved cosines.
...
/// Monic whitening-filter coefficients (Q12) from NLSFs (Q15); `d` is 10 or
/// 16.
...
mod tests {
    ...
    /// Reference pins for the NLSF chain (decode → NLSF2A, with
    /// stabilisation and the inverse-prediction-gain loop).
    #[test]
    ...
    /// the sort fallback), recorded from the reference.
    #[test]
